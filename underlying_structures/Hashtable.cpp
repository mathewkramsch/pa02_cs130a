...
node* Hashtable::insert(int i) { 
	...
	if (!n) hashArray[i%43] = new node(i,1,i%43);  // if no node exists, then insert new node into hashArray
	else while (n) {  // traverse through linkedlist until correct node found
		if (n->value==i) {  		
			n->count++;  // increment node in hashArray;
			break;
		}
		else n=n->next;  // else go to next node in linkedlist
	}
	if (!n) n = new node(i,1,i%43);  // if node != exist add new node to end of linkedlist at hashArray[index]
		// n must be either head of empty or tail of linkedlist
	
	cout << "\tinserted " << i << " to hashArray at index: " << i%43 << endl;
	return n; 
}
...
node* Hashtable::lookup(int i) { 
	...
	while (n) {  // traverse through linkedlist at hashArray[index], search for node w/ value;
		if (n->value == i) { 
			cout << "\tfound" << endl; 
			if (!n->link) cout << "\tn->link==nullptr" << endl;
			return n; 
		}
		n = n->next;
	}
